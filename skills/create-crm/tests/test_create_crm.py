...
def setup_module():
    FIXTURE.write_text(json.dumps({"intention": "crm simples para clientes"}, ensure_ascii=False), encoding="utf-8")
...
def test_output_schema():
    ...
    output = json.loads(result.stdout)
    for key in ["prd_path", "blueprint_path", "frontend_plan", "acceptance_criteria", "entities", "next_action"]:
