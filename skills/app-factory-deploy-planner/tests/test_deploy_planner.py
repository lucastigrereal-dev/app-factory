"""Testes de fumaça para app-factory-deploy-planner."""
...
def test_dry_run_outputs_deploy_plan():
    ...
    assert "deploy_plan" in output
    assert "rollback_plan" in output
...
def test_output_schema():
    ...
    input_file.write_text(json.dumps({"stack": {}}), encoding="utf-8")
    ...
    output = json.loads(result.stdout)
    required = ["deploy_plan", "github_actions", "env_vars", "rollback_plan", "estimated_time", "next_action"]
    for key in required:
