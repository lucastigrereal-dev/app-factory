"""Smoke tests for memory-writeback."""
import json
import subprocess
import sys
...
def setup_module():
    FIXTURE.write_text(json.dumps({"records": [{"insight": "Pipeline rapido", "priority": "high"}]}, ensure_ascii=False), encoding="utf-8")
...
def test_dry_run_completes():
    ...
    assert "writeback_plan" in output
